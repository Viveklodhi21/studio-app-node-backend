...
export const createShopAdmin = async (req: Request, res: Response) => {
  const { userId, shopName, shopOwnerName, shopEmail, shopAddress, shopCity, shopPhone, role } =
    req.body;
  ...
  try {
    const data: any = new Shop(userData);
...
export const editShop = async (req: Request, res: Response) => {
  const {
    userId,
    shopName,
    shopOwnerName,
